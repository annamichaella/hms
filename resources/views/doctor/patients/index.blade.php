...
@section('content')
    <div class="mb-6">
        <h1 class="text-xl font-bold text-gray-800 tracking-tight">My Patients</h1>
        <p class="text-gray-600 text-sm mt-1">Search and manage your patients</p>
    </div>
        ...
        <table class="w-full min-w-full">
            <thead class="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Last Appointment</th>
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($patients as $patient)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $patient->patientAppointments->first()?->appointment_date->format('M d, Y') ?? 'N/A' }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            ...
                            @endphp
                            @if($patientRecord)
                                <a href="{{ route('doctor.patients.records', $patient->id) }}" class="text-blue-600 hover:text-blue-900">View Records</a>
                            @else
    ...
    <script>
        new AutoSearch({
            ...
            searchUrl: '/doctor/patients/search',
            onSearch: function(data) {
                ...
                tbody.innerHTML = data.map(patient => {
                    const lastAppointment = patient.last_appointment ? new Date(patient.last_appointment).toLocaleDateString('en-US', {month: 'short', day: 'numeric', year: 'numeric'}) : 'N/A';
                    ...
                    return `
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                ${hasRecord ? 
                                    `<a href="/doctor/patients/${patient.id}/records" class="text-blue-600 hover:text-blue-800 transition-colors">View Records</a>` :
                                    `<span class="text-gray-400">No Records</span>`
                                }
                            </td>
                        </tr>
